...
/*
	정렬과 관련된 interface는 Comparable, Comparator 이렇게 두 가지가 있다.
 	...
 	Comparable은 Collection에 추가되는 데이터 자체의 정렬기준을 넣고 싶을 때 구현하는 인터페이스이다. ==> 내부 정렬 기준
 	
 	Comparator는 외부에 별도로 정렬 기준을 구현하고 싶을 때 사용한다. ==> 외부 정렬 기준
 	
 	Comparable에서는 comparaTo()메서드를 재정의하고,
...
class Desc implements Comparator<String> {
	...
	/*
		...
		예) 오름차순의 경우 ==> 앞의 값이 크면 양수, 같으면 0, 앞의 값이 작으면 음수를 반환하도록 구현하면 된다.
	*/
...
//		return 0;
		...
		return str1.compareTo(str2) * -1; // 위의 값과 같은 결과를 얻을 수 있음
	}
