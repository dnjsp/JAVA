...
public class ThreadTest142 {
	public static Random rnd = new Random();
	public static ArrayList<Horse2> horses = new ArrayList<>();
	public static int rank = 1;

	public static void main(String[] args) {
		for(int i = 1; i<= 10; i++) {
			horses.add(new Horse2(String.format("%02d번 말", i)));
		}
...
class DisplayHorseRacing extends Thread {
	...
	public void run() {
		...
		do {
			endRacingHorse = 0;
			System.out.println();
			for(Horse2 horse : ThreadTest142.horses) {
		...
		}while(endRacingHorse < ThreadTest142.horses.size());
	}
}
...
class Horse2 extends Thread implements Comparable<Horse2> {
	...
	public void run() {
		for(int i = 0; i < 50; i++) {
			this.position = i;
			try {
				Thread.sleep(ThreadTest142.rnd.nextInt(300));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		this.rank = ThreadTest142.rank ++;
	}
