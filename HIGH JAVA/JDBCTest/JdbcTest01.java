...
//JDBC (java DataBase Connectivity) 라이브러리를 이용해서 DB자료 처리하기 
public class JdbcTest01 {
	/*
	 * JDBC를 이용한 DB자료 처리 순서 
	 * 1. 드라이버 로딩 ==> 라이브러리를 사용할 수 있게 메모리에 읽어들이는 작업 
	 * 		class.forName("oracle.jdbc.driver.OracleDriver"); 
	 * 		=> JDBC버전이 4이상에서는 getConnection()메서드에서 자동으로 로딩해준다. 
	 * 		   그렇지만 우리는 생략하지 않고 사용할 예정.. 
	 * 
	 ...
	 * 		2) 실행한 SQL문이 select문이 아닐 경우(insert, update, delete)에는 정수값이 반환된다. 
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 3-2. Statement객체 생성 ==> Connection객체를 이용해서 생성 
