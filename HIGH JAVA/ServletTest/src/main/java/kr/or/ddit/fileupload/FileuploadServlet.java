...
/*
    - Servlet 3.0이상에서 파일을 업로드하려면 서블릿에 @MultipartConfig 애노테이션을 설정해야 한다.
    - @MultipartConfig 애노테이션에서 설정할 변수들
      1) location : 업로드한 파일이 임시적으로 저장될 경로 지정 : (기본값 : "")
      2) fileSizeThreshold : 이 속성에 설정한 값보다 큰 파일이 전송되면 location에 지정한 임시 디렉토리에 저장한다.
          (단위 : byte, 기본값 : 0 (무조건 임시디렉토리에 저장된다.))
      3) maxFileSize : 1개의 파일의 최대 크기 설정 (단위 : byte, 기본값 : -1L (무제한))
      4) maxRequestSize : 서버로 전송되는 전체 Request데이터의 최대 크기 설정 (단위 : byte, 기본값 : -1L (무제한)) ==> (파일 + formData)
...
public class FileuploadServlet extends HttpServlet {
   ...
   // 파일 전송에는 POST방식만 사용 가능하다.
   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      // 사용자가 업로드한 파일이 저장될 서버쪽의 폴더 경로 설정
      String uploadPath = "C:\\ddit\\D_Other\\uploadFiles";
      ...
      // 파일이 아닌 일반 Form Data들은 getParameter()메서드나 getParameterValues()메서드를 이용해서 구한다.
       ...
       * 
       ...
       - Part객체에서 제공하는 메서드들..
         ...
         3) getHeader(String name) : Part로부터 지정한 name을 갖는 헤더값을 반환한다.
         4) write(String fileName) : 임시 저장되어 있는 파일 데이터를 fileName에 지정한 경로로 저장한다.
   ...
   /*
    ...
    1) 파일이 아닐 때 (즉, 일반 Form Data일 경우)
       -------------ajfkrhrmfTlrkTjwudlTdma              ==> 각각의 Part를 구분하는 구분선
   ...
   private String extractFileName(Part part) {
      ...
      String[] items = contentDispositon.split(";");
