...
public class CookieDelete extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 저장된 쿠키 중에서 삭제할 쿠키를 찾아서 이 쿠키의 남은 수명을 0으로 설정한 후에 다시 저장하는 방법을 사용한다.
		// 설정한 후에 다시 저장하는 방법을 사용한다. ==> setMaxAge()메서드 사용
		...
		out.println("<body>");
		out.println("<h2>저장된 Cookie 데이터 삭제하기</h2><br>");
		...
		if (cookieArr == null || cookieArr.length == 0) {
			...
			for (Cookie cookie : cookieArr) {
				...
				if ("gender".equals(name)) {  // 삭제할 쿠키 찾기
					// 찾은 쿠키의 수명을 0으로 설정한다.
					cookie.setMaxAge(0);
