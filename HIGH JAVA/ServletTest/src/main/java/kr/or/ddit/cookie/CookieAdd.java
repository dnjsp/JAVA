...
public class CookieAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 		 ==> 쿠키값으로 한글을 사용할 경우에는 URLEncoder.encode()메서드로 인코딩한 후 값을 저장한다.
		
		// 하나의 쿠키는 4kB(4096 byte)까지 저장 가능
		// 하나의 도메인당 20개, 총 300개까지 저장 가능
		...
		/*
		 	2. 쿠키 속성 설정
			- 쿠키변수.setPath("적용경로"); ==> 지정한 경로와 그 하위 경로에서만 사용가능하다.
									   	    ==> 생략하면 쿠키를 저장할 당시의 경로가 설정된다.
			- 쿠키변수.setMaxAge(유지시간); ==> 단위 : 초 (값이 -1이면 웹브라우저가 종료될 때 삭제되고, 값이 0이면 즉시 삭제된다.)
		...
		out.println("<a href='" + request.getContextPath() +
				"/02/cookieTest.jsp'>시작문서로 이동하기</a>");
		out.println("");
