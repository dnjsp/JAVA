	...
	public static void main(String[] args) {
		try {
			...
			while (true) {
				...
				// 수신 받은 패킷객체를 이용하여 상대방의 주소, 포트번호등을 알 수 있다.
				...
				// 송신한 메세지를 byte형 배열로 변환한다.
				byte[] sendMsg = msg.getBytes("utf-8");
				
				// 송신용 패킷객체 생성하기
				// ==> 전송할 데이터가 저장된 byte형 배열,
				// 	   전송할 자료의 길이(배열의 길이), 상대방 주소 정보, 포트번호
				// ==> 위 4가지를 지정하여 생성한다.
